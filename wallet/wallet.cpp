    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            for (unsigned int i = 0; i < vValue.size(); i++)
            {
                ...
                //that the rng is fast. We do not use a constant random syncoin,
                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int nSplitBlock, std::string& strFailReason, const CCoinControl* coinControl)
{
    ...
    // Syncoin: define some values used in case of namecoin tx creation
    ...
    txNew.nVersion = wtxNew.nVersion; // Syncoin: important for name transactions

    {
        LOCK2(cs_main, cs_wallet);
        {
            nFeeRet = max(nFeeInput, MIN_TX_FEE);  // Syncoin: a good starting point, probably...
            while (true)
            {
                txNew.vin.clear();
                txNew.vout.clear();
                ...
                // Syncoin: in case of namecoin tx we have already supplied input.
                ...
                // otherwise proceed as we normaly would in syncoin
                else
        ...
        // Syncoin: name tx always at first position
                if (!wtxNameIn.IsNull())
                {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-syncoin-address
                ...
                for(const std::pair<const CWalletTx*, unsigned int>& coin : setCoins)
                {
                    // Syncoin: we sign name tx differently.
